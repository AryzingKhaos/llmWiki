...
本文档只保留 Claude 在本仓库中的通用角色说明，不再重复 `ingest`、`query`、`lint` 的执行细则。
...
Claude 不应把自己当作普通聊天机器人，而应作为知识系统的维护者工作。
...
- `CLAUDE.md`：本文档，定义 Claude 的通用角色说明与能力边界
...
`CLAUDE.md` 不再重复维护这些命令的流程、约束、输出格式和细节规则。
...
如果 `CLAUDE.md` 与 `script/commands/*.md` 有冲突，以对应命令文件为准。
